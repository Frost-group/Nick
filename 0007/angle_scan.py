...
def main():
	...
	plt.plot(Gscan['Angle'], Gscan['E_mev'])
	...
	MD_tot_energy = pd.read_table("angle.xvg",skiprows=1000, delim_whitespace=True, header=None, nrows=43000)
	...
	for i in range(0,42):
		j = i*1000
		binned.loc[i,'angle'] = MD_tot_energy['angle'].iloc[j:j+1000].mean(axis=0)
		binned.loc[i,'energy'] = MD_tot_energy['E_kjmol'].iloc[j:j+1000].mean(axis=0)
