...
a <- read.table("GM_cryst_merc.tsv")
a <- as_tibble(data.frame(q=(4*pi / 1.54)*sin(a$V1*0.0174533 /2),S=a$V2, method="Simulated")) 

lab <- as_tibble(read.table("GM_cryst_merc.hkl", header=TRUE)) %>% select(-c("multiplicity")) %>% 
	mutate(q= 2*pi / d.spacing) %>% select(-c("d.spacing")) %>%
	mutate(label=paste("(",h," ", k," ", l,")", sep="")) %>%  
	dplyr::filter((F.2 > 4000 & q<2)  | label=="(0 1 0)" | label=="(1 0 0)" | label=="(0 2 0)"| label=="(0 3 0)") 
...
ggsave("GM_cryst_merc.pdf",plot1,width=11)
